...
def ssti_cheatsheet():
  ...
  {C.YELLOW}JINJA2 (Python / Flask) — most common in CTFs{C.RESET}
    ...
    {{{{''.__class__.__mro__}}}}          → find class hierarchy
    {{{{''.__class__.__mro__[1].__subclasses__()}}}}   → list all classes (find index of Popen)
...
╚══════════════════════════════════════════════════════════════╝{C.RESET}
  ...
  {C.CYAN}── CRYPTOGRAPHY ────────────────────────────────────────────{C.RESET}
   ...
   2.  Base32               30.  ROT47                  {C.DIM}[new]{C.RESET}
   3.  Base58               31.  Atbash                 {C.DIM}[new]{C.RESET}
   4.  Hex                  32.  Rail Fence Cipher       {C.DIM}[new]{C.RESET}
   5.  Binary               33.  Bacon Cipher            {C.DIM}[new]{C.RESET}
   6.  ROT13                34.  Base85 / Ascii85        {C.DIM}[new]{C.RESET}
   7.  Caesar Brute Force   35.  Affine Cipher           {C.DIM}[new]{C.RESET}
   8.  Vigenère             36.  Columnar Transposition  {C.DIM}[new]{C.RESET}
   9.  XOR                  37.  Hash Cracker (wordlist) {C.DIM}[new]{C.RESET}
  10.  Frequency Analysis   38.  Polybius Square         {C.DIM}[new]{C.RESET}
  ...
  {C.CYAN}── MACHINES (HTB / THM) ────────────────────────────────────{C.RESET}
  55.  Nmap Command Builder              {C.DIM}[new]{C.RESET}
  56.  Reverse Shell Generator           {C.DIM}[new]{C.RESET}
  57.  Web Enumeration Reference         {C.DIM}[new]{C.RESET}
  58.  Privilege Escalation Reference    {C.DIM}[new]{C.RESET}
  59.  SMB Enumeration Reference         {C.DIM}[new]{C.RESET}
  60.  Password Attacks Reference        {C.DIM}[new]{C.RESET}
  61.  Programming Challenges            {C.DIM}[new]{C.RESET}
  62.  CTF Methodology Checklist         {C.DIM}[new]{C.RESET}

  {C.CYAN}── TOOL EXTENSIONS (prerequisite tools) ────────────────────{C.RESET}
  63.  RSA Factorization  (find p, q from n)     {C.DIM}[new]{C.RESET}
  64.  Vigenère Key Recovery (no key needed)      {C.DIM}[new]{C.RESET}
  65.  JWT HMAC Secret Brute Force                {C.DIM}[new]{C.RESET}
